  ...
  async ngOnInit() {
    ...
    for (let i = 0; i < response.entries.length; i++) {
      ...
      card.onclick = async () => {
        ...
        let file = new File([fileResponse["result"].fileBlob], "book.epub");
      ...
      let headers = {
        ...
        headers: {
          "Authorization": "Bearer " + acsess_token,
          'Content-Type': 'text/plain',
          "Dropbox-API-Arg": JSON.stringify({ "path": entry.path_lower, "size": "w256h256"}),
        },
      }
      ...
      let imageFile = new File([imageFileResponse["result"].fileBlob], "pic.jpeg");
